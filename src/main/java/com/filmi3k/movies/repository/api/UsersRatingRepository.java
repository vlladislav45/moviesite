...
public interface UsersRatingRepository extends JpaRepository<UsersRating, Integer> {
    ...
    @Query(value = "SELECT sum(user_rating) FROM users_rating WHERE movie_id=?1",
            nativeQuery=true)
    double sumAllUsersRatingsByMovie(int movieId);

    @Query(value = "SELECT COUNT(user_rating) FROM users_rating WHERE movie_id=?1",
            nativeQuery=true)
